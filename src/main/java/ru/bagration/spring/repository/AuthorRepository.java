...
public interface AuthorRepository extends JpaRepository<Author, Long>, JpaSpecificationExecutor<Author> {
    ...
    @Query("select t.firstName,t.secondName from Author t where t.id = :id")
    String findNameById(Long id);

    @Query("select t.publicId from Author t where t.id = :id")
    String findPublicIdById(Long id);

    @Query("select a.id from Author a where lower(a.firstName) like :name or lower(a.secondName) like :name")
